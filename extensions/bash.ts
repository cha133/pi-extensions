...
export const BASH_PROMPT_GUIDELINES = [
	[
		"The bash tool runs pwsh7. Write PowerShell 7 syntax, not bash/sh:",
		"set environment variables with `$env:NAME = 'x'` (not `export`),",
		"continue lines with a backtick (not `\\`),",
		"test paths with `Test-Path` (not `[ -f ]`),",
		"and invoke executables whose paths contain spaces with",
		"`& 'C:\\path\\app.exe' arg`.",
	].join(" "),
	[
		"For searching, prefer `rg`: use `rg --files` to list files",
		"(for example, `rg --files -g '*.md'` for Markdown files, not `find`)",
		"and `rg PATTERN PATH` to search content (not `grep` or `Select-String`).",
		"`rg --files --hidden` includes hidden files while still respecting ignore files;",
		"add `--no-ignore` to also include ignored files and directories,",
		"normally excluding repository metadata with",
		"`rg --files --hidden --no-ignore -g '!**/.git/**'`.",
		"Limit output with `Select-Object -First N` / `-Last N`",
		"(not `head` / `tail`; `Get-Content -TotalCount N` / `-Tail N` also works),",
		"count lines with `(Get-Content f | Measure-Object -Line).Lines` (not `wc -l`),",
		"and locate commands with `(Get-Command name).Source` (not `which`).",
	].join(" "),
	[
		"ripgrep searches directories recursively by default.",
		"Do not use grep-style `rg -r` or `rg -rn`:",
		"in ripgrep, `-r` means `--replace`,",
		"so `rg -rn PATTERN PATH` replaces matches with `n`.",
		"Use `rg -n PATTERN PATH` for recursive search with line numbers.",
	].join(" "),
	[
		"Pass multiline arguments to native executables, such as Git commit messages,",
		"with a here-string: `@'...'@` does not expand variables,",
		"while `@\"...\"@` does.",
		"The closing marker `'@` / `\"@` must begin at column 1",
		"or PowerShell will report an error.",
	].join(" "),
