...
export const BASH_PROMPT_GUIDELINES = [
	...
	"For searching, prefer `rg`: use `rg --files` to list files (for example, `rg --files -g '*.md'` for Markdown files, not `find`) and `rg PATTERN PATH` to search content (not `grep` or `Select-String`). `rg --files --hidden` includes hidden files while still respecting ignore files; add `--no-ignore` to also include ignored files and directories, normally excluding repository metadata with `rg --files --hidden --no-ignore -g '!**/.git/**'`. Limit output with `Select-Object -First N` / `-Last N` (not `head` / `tail`; `Get-Content -TotalCount N` / `-Tail N` also works), count lines with `(Get-Content f | Measure-Object -Line).Lines` (not `wc -l`), and locate commands with `(Get-Command name).Source` (not `which`).",
