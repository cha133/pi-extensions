...
Omitting `tier` always selects `peer`. The `advisor` option is exposed only when
its configured model exists and differs from the current main model; selecting
that same model as the main model hides the option again. This keeps normal
