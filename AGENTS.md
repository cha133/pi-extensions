...
| `read` | read.ts | **Overrides built-in.** Wraps `createReadToolDefinition`; text results become `[PATH#16HEXTAG]` plus `LINE:TEXT` rows consumed by `edit`, and displayed line ranges are recorded for the current session. Native image handling remains intact, and models without image input route through the `vision` model selected in `~/.pi/agent/settings.json`. Optional `image.query` and `image.detail` parameters guide targeted fallback analysis. |
